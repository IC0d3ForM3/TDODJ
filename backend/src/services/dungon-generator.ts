...
const GRID_SIZES: Record<string, number> = { small: 3, medium: 4, large: 5 };

// ── Layout types ──────────────────────────────────────────────────────────────
interface LayoutRoom {
  id: string; gi: number; gj: number;
  rowStart: number; rowEnd: number; colStart: number; colEnd: number;
}

type ConnType = 'open' | 'door' | 'locked';

interface LayoutCorridor {
  roomAId: string; roomBId: string;
  direction: 'horizontal' | 'vertical';
  corridorRow: number; corridorCol: number;
  connectionType: ConnType;
}

interface Layout { rooms: LayoutRoom[]; corridors: LayoutCorridor[]; }

// ── AI blueprint types ────────────────────────────────────────────────────────
interface AIRoomContent {
  id: string; name: string; description: string;
  isStartRoom: boolean; startFacing?: 'up' | 'right' | 'down' | 'left';
  hasExit: boolean; exitType?: 'open' | 'stairsUp' | 'stairsDown';
  monsterDbIds: number[];
  treasureGold: number; treasureSilver: number; treasureCopper: number;
  hasLoot: boolean; lootDescription: string;
  floorItemDbIds: number[]; floorPotionDbIds: number[]; floorSpellDbIds: number[];
  obstacleDescription: string | null;
  signText: string | null;
}

interface AICorridorContent {
  roomAId: string; roomBId: string;
  type: 'open' | 'door' | 'locked';
  doorName?: string; keyName?: string;
  keyInRoom?: string | null;
  keyOnFirstMonsterInRoom?: string | null;
}

interface AIPortalContent {
  fromRoomId: string; toRoomId: string;
  isTwoWay: boolean; look: 'starUp' | 'starDown' | 'magicDoor'; name: string;
}

interface AIBlueprint {
  rooms: AIRoomContent[];
  corridors: AICorridorContent[];
  portals?: AIPortalContent[];
}

// ── Payload types (match frontend interfaces) ─────────────────────────────────
interface Wall { id: number; name: string; description: string; HP: number; state: 'intact'; isDestructible?: boolean; }
interface Door { id: number; name: string; description: string; keyLock: Key | null; isLocked: boolean; isTrapped: boolean; toPick: number | null; trap: null; HP: number; state: 'closed'; isHidden: boolean; toFind: number; isFound: boolean; spReward: null; itemRequirement: null; }
interface Key { id: number; name: string; description: string; doorId: number | null; rownId: number | null; columnId: number | null; }
type SquareSide = Wall | Door | null;
interface Square { id: number; row: number; column: number; description: string; isTrapped: boolean; toTop: SquareSide; toRight: SquareSide; toBottom: SquareSide; toLeft: SquareSide; }

// ── DB record types ───────────────────────────────────────────────────────────
interface DBMonster {
  id: number; name: string; type: string; hp: number; ac: number;
  description: string;
  movementEconomy: number; runAt: number; numberOfAttacks: number;
  attacks: unknown[]; spReward: number; magic: number; magicResistance: number;
  castPlus: number;
  callsReinforcements: boolean; toHitPlusNeeded: number;
  reinforcementCount: number; reinforcementMonsterName: string | null;
  imageId: number | null; soundId: number | null;
  tresherIds: number[]; keyIds: number[];
  npcGreeting: string | null; npcInfo1: string | null; npcInfo2: string | null; npcInfo3: string | null;
  npcOnlyAttackWhenAttacked: boolean; npcGivesInfoAfterDamaged: boolean;
  npcAttacksAfterInfo: boolean; npcCanTrade: boolean; awareness: number;
}
interface DBItem {
  ...
  effectToPc: string | null; effectToPcValue: number;
}
...
// ── Public params ─────────────────────────────────────────────────────────────
export interface GenerateDungonParams {
  name: string;
  story: string;
  level: number;
...
// isn't a pure tree.
function generateLayout(gridSize: number): Layout {
  // ── 1. Choose per-row heights and per-column widths ──────────────────────
  const DIM_OPTIONS = [2, 3, 3, 3, 4, 4, 5]; // 3 is most common, 5 is rare
  const rowHeights: number[] = Array.from({ length: gridSize }, () =>
    DIM_OPTIONS[Math.floor(Math.random() * DIM_OPTIONS.length)]);
  const colWidths: number[] = Array.from({ length: gridSize }, () =>
    DIM_OPTIONS[Math.floor(Math.random() * DIM_OPTIONS.length)]);
  ...
  const connectedPairs = new Set<string>(); // track existing connections

  function makeCorridor(roomA: LayoutRoom, roomB: LayoutRoom, gi: number, gj: number, ni: number, nj: number): void {
    let corridorRow: number, corridorCol: number, direction: 'horizontal' | 'vertical';
    if (ni === gi) {
  ...
  function dfs(gi: number, gj: number): void {
    visited.add(`${gi}_${gj}`);
    const neighbors: [number, number][] = [[gi - 1, gj], [gi + 1, gj], [gi, gj - 1], [gi, gj + 1]]
      .filter(([r, c]) => r >= 0 && r < gridSize && c >= 0 && c < gridSize) as [number, number][];
    for (let i = neighbors.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [neighbors[i], neighbors[j]] = [neighbors[j], neighbors[i]];
    }
    for (const [ni, nj] of neighbors) {
      if (visited.has(`${ni}_${nj}`)) continue;
  ...
  for (let gi = 0; gi < gridSize; gi++) {
    for (let gj = 0; gj < gridSize; gj++) {
      const neighbors: [number, number][] = [[gi, gj + 1], [gi + 1, gj]]
        .filter(([r, c]) => r < gridSize && c < gridSize) as [number, number][];
      for (const [ni, nj] of neighbors) {
...
function makeDoor(id: number, name: string, isLocked: boolean, keyLock: Key | null): Door {
...
function squareKey(r: number, c: number): string { return `${r}:${c}`; }

// ── DB Fetchers ───────────────────────────────────────────────────────────────
async function fetchPublicMonsters(): Promise<DBMonster[]> {
  const { rows } = await pool.query<DBMonster>(`
...
async function fetchPublicItems(): Promise<DBItem[]> {
  const { rows } = await pool.query<DBItem>(`
...
async function fetchPublicPotions(): Promise<DBPotion[]> {
  const { rows } = await pool.query<DBPotion>(`
...
async function fetchPublicSpells(): Promise<DBSpell[]> {
  const { rows } = await pool.query<DBSpell>(`
    SELECT id, name, description,
...
// ── OpenAI Call ───────────────────────────────────────────────────────────────
async function callOpenAI(layout: Layout, params: GenerateDungonParams, monsters: DBMonster[], items: DBItem[], potions: DBPotion[], spells: DBSpell[]): Promise<AIBlueprint> {
  const apiKey = process.env['OPENAI_API_KEY'];
  if (!apiKey) throw new Error('OPENAI_API_KEY is not configured in the backend .env file.');
...
Return this exact JSON schema (fill in ALL ${layout.rooms.length} rooms):
{
  ...
  return JSON.parse(content) as AIBlueprint;
}

// ── Blueprint Validation ──────────────────────────────────────────────────────
function validateAndRepair(blueprint: AIBlueprint, layout: Layout, monsters: DBMonster[], items: DBItem[], potions: DBPotion[], spells: DBSpell[]): void {
  ...
  if (startRooms.length === 0) { blueprint.rooms[0].isStartRoom = true; blueprint.rooms[0].startFacing = 'right'; }
  else if (startRooms.length > 1) { startRooms.slice(1).forEach(r => { r.isStartRoom = false; }); }
  ...
  for (const c of blueprint.corridors) {
    if (c.type === 'locked') {
      if (!c.keyName) c.keyName = 'Iron Key';
...
// ── Payload Builder ────────────────────────────────────────────────────────────
export function buildDungonPayload(layout: Layout, blueprint: AIBlueprint, monsters: DBMonster[], items: DBItem[], potions: DBPotion[], spells: DBSpell[]): Record<string, unknown> {
  ...
  for (const corridor of layout.corridors) {
    ...
    if (connType === 'door' || connType === 'locked') {
      ...
      if (connType === 'locked') {
        ...
        const key: Key = { id: nextKeyId++, name: keyName, description: `Opens the passage between ${corridor.roomAId} and ${corridor.roomBId}`, doorId: null, rownId: null, columnId: null };
        keyList.push(key);
      ...
      if (keyLock) (keyLock as { doorId: number | null }).doorId = door.id;
    }
  ...
  for (const corridor of layout.corridors) {
    if (corridor.connectionType !== 'locked') continue;
    const aiKey = `${corridor.roomAId}|${corridor.roomBId}`;
    const aiCorridor = aiCorridorMap.get(aiKey);
    const key = corridorKeyMap.get(aiKey);
    if (!key || !aiCorridor) continue;

    const targetRoomId = aiCorridor.keyInRoom ?? null;
    if (targetRoomId) {
      const room = roomMap.get(targetRoomId);
      if (room) { key.rownId = room.rowStart; key.columnId = room.colStart; }
    }
    // If keyOnFirstMonsterInRoom: key stays on the monster (rownId null), handled in monster section
  }

  // ── Monsters ─────────────────────────────────────────────────────────────────
  const monsterList: unknown[] = [];
  const monsterPlacements: unknown[] = [];
  const dbIdToLocalId = new Map<number, number>();
  const allUsedDbIds = new Set<number>(blueprint.rooms.flatMap(r => r.monsterDbIds ?? []));
  ...
  const roomMonsterKeyMap = new Map<string, number[]>();
  for (const corridor of layout.corridors) {
    if (corridor.connectionType !== 'locked') continue;
    const aiKey = `${corridor.roomAId}|${corridor.roomBId}`;
    const aiCorridor = aiCorridorMap.get(aiKey);
    const key = corridorKeyMap.get(aiKey);
    if (!key || !aiCorridor?.keyOnFirstMonsterInRoom || aiCorridor.keyInRoom) continue;
  ...
  const placedMonsterPositions = new Set<string>();
    ...
    for (let idx = 0; idx < monsterDbIds.length; idx++) {
      ...
      for (let r = room.rowStart; r <= room.rowEnd && placeRow === -1; r++) {
        for (let c = room.colStart; c <= room.colEnd && placeRow === -1; c++) {
          if (!placedMonsterPositions.has(squareKey(r, c))) { placeRow = r; placeCol = c; }
        }
      }
      if (placeRow === -1) continue;
      ...
      if (assignedKeys.length > 0) {
        const monEntry = (monsterList as {id:number; keyIds: number[]}[]).find(m => m.id === localId);
        if (monEntry) monEntry.keyIds = [...(monEntry.keyIds ?? []), ...assignedKeys];
        ...
        for (const kid of assignedKeys) {
          ...
          if (k) { k.rownId = null; k.columnId = null; }
        }
      }
  ...
  // ── Treshers ─────────────────────────────────────────────────────────────────
  const tresherList: unknown[] = [];
  const tresherPlacements: unknown[] = [];
  for (const aiRoom of blueprint.rooms) {
    if (!aiRoom.hasLoot) continue;
    const room = roomMap.get(aiRoom.id);
    if (!room) continue;
    ...
    if (placedMonsterPositions.has(squareKey(tRow, tCol))) tCol = room.colStart;
    tresherPlacements.push({ tresherId: tid, row: tRow, column: tCol });
  }

  // ── Floor items / potions / spells ────────────────────────────────────────────
  const itemPlacements: unknown[] = [];
  const potionPlacements: unknown[] = [];
  const spellPlacements: unknown[] = [];
  const floorItemList: unknown[] = [];
  const floorPotionList: unknown[] = [];
  const floorSpellList: unknown[] = [];
  const seenItemIds = new Set<number>(), seenPotionIds = new Set<number>(), seenSpellIds = new Set<number>();
  ...
  for (const aiRoom of blueprint.rooms) {
    ...
    for (const potionId of (aiRoom.floorPotionDbIds ?? [])) {
      const dbP = potionDbMap.get(potionId); if (!dbP) continue;
      ...
      if (!seenPotionIds.has(potionId)) { seenPotionIds.add(potionId); floorPotionList.push({ id: dbP.id, name: dbP.name, description: dbP.description, effectTo: dbP.effectTo, effectAmount: dbP.effectAmount, lastFor: dbP.lastFor }); }
      offset++;
    }
    offset = 0;
    for (const spellId of (aiRoom.floorSpellDbIds ?? [])) {
      const dbS = spellDbMap.get(spellId); if (!dbS) continue;
      ...
      if (!seenSpellIds.has(spellId)) { seenSpellIds.add(spellId); floorSpellList.push({ id: dbS.id, name: dbS.name, description: dbS.description, range: dbS.range, effectOn: dbS.effectOn, effectAmount: dbS.effectAmount, successTestValue: dbS.successTestValue, sp: dbS.sp, lastFor: dbS.lastFor, numberOfTargets: dbS.numberOfTargets, magicCost: dbS.magicCost }); }
      offset++;
    }
  }

  // ── Obstacles ─────────────────────────────────────────────────────────────────
  const obstaclePlacements: unknown[] = [];
  for (const aiRoom of blueprint.rooms) {
    if (!aiRoom.obstacleDescription) continue;
    const room = roomMap.get(aiRoom.id); if (!room) continue;
  ...
  // ── Portals ────────────────────────────────────────────────────────────────────
  const portalPlacements: unknown[] = [];
  for (const aiPortal of (blueprint.portals ?? [])) {
    const fromRoom = roomMap.get(aiPortal.fromRoomId); const toRoom = roomMap.get(aiPortal.toRoomId);
  ...
  const squareTexts: unknown[] = [];
  for (const aiRoom of blueprint.rooms) {
    if (!aiRoom.signText) continue;
    const room = roomMap.get(aiRoom.id); if (!room) continue;
  ...
  for (const aiRoom of blueprint.rooms) {
    ...
    if (sq) sq.description = `${aiRoom.name}: ${aiRoom.description}`;
  }
...
interface ExistingSquareRecord {
  id?: number;
  row: number;
  column: number;
  description?: string;
...
interface ExistingStartPointRecord {
  row: number;
  col: number;
  description: string;
  playerSees: string;
...
interface ExistingExitRecord {
  ...
  itemRequirement: unknown | null;
}

interface ParsedExistingDungonPayload {
  basePayload: Record<string, unknown>;
  filledSquares: Record<string, true>;
  squares: Record<string, ExistingSquareRecord>;
  startpoint: ExistingStartPointRecord;
  exits: ExistingExitRecord[];
  keyList: unknown[];
  cheater: Record<string, unknown>;
...
interface AIFillMonsterPlacement {
  square: string;
  monsterDbId: number;
}

interface AIFillTreasurePlacement {
  square: string;
  name: string;
  description: string;
  gold: number;
...
interface AIFillObstaclePlacement {
  square: string;
  name: string;
  note?: string;
}

interface AIFillNotePlacement {
  square: string;
  text: string;
}

interface AIFillBlueprint {
  startDescription?: string;
  startPlayerSees?: string;
  monsterPlacements: AIFillMonsterPlacement[];
  treasures: AIFillTreasurePlacement[];
  obstacles: AIFillObstaclePlacement[];
  notes: AIFillNotePlacement[];
}
...
function parseFilledSquares(source: Record<string, unknown>, squares: Record<string, ExistingSquareRecord>): Record<string, true> {
  const filledSquaresSource = asObject(source['filledSquares']);
  const filledSquares = Object.keys(filledSquaresSource).reduce<Record<string, true>>((accumulator, key) => {
...
function parseExistingDungonPayload(rawDungonJson: unknown): ParsedExistingDungonPayload {
  ...
  const squares = Object.entries(rawSquares).reduce<Record<string, ExistingSquareRecord>>((accumulator, [key, value]) => {
  ...
  const startpoint: ExistingStartPointRecord = {
    row: startRow,
    col: startCol,
  ...
  const rawExits = Array.isArray(basePayload['exits'])
    ...
    : Array.isArray(basePayload['exitList'])
      ...
      : [];
  const exits = rawExits.reduce<ExistingExitRecord[]>((accumulator, value, index) => {
...
function isBarrier(side: unknown, includeDoors: boolean): boolean {
...
function getNeighborKeys(
  square: ExistingSquareRecord,
  squares: Record<string, ExistingSquareRecord>,
  options?: { includeDoorsAsBarriers?: boolean },
): string[] {
  ...
  for (const step of DIRECTION_STEPS) {
    ...
    if (
      isBarrier(square[step.side], includeDoorsAsBarriers)
...
function buildDistanceMap(
  payload: ParsedExistingDungonPayload,
  originKey: string,
  options?: { includeDoorsAsBarriers?: boolean },
): Map<string, number> {
  ...
  const distanceMap = new Map<string, number>([[originKey, 0]]);
...
function buildConnectedOpenAreaKeys(
  payload: ParsedExistingDungonPayload,
  anchorKey: string,
): Set<string> {
...
function buildPlacementCandidates(
  payload: ParsedExistingDungonPayload,
  options?: { distanceOriginKey?: string; allowedSquareKeys?: Set<string>; blockedSquareKeys?: Set<string> },
): PlacementCandidate[] {
  ...
  return Object.entries(payload.squares)
    ...
    .map(([key, square]) => {
      ...
      const tags: string[] = [];
      if (openness >= 3) tags.push('chamber');
      if (openness === 1) tags.push('dead-end');
      if (distanceFromStart <= 3) tags.push('near-start');
      if (distanceFromStart >= Math.max(4, furthestDistance - 2)) tags.push('late-area');
      if (openness <= 2) tags.push('corridor');
...
function collectOccupiedSquareKeys(basePayload: Record<string, unknown>): Set<string> {
...
function trimCandidates(candidates: PlacementCandidate[]): PlacementCandidate[] {
  if (candidates.length <= 120) {
    return candidates;
  }

  const trimmed: PlacementCandidate[] = [];
...
async function callOpenAIToFillExistingDungon(
  payload: ParsedExistingDungonPayload,
  params: GenerateDungonParams,
  monsters: DBMonster[],
  candidates: PlacementCandidate[],
  monsterCandidates: PlacementCandidate[],
): Promise<AIFillBlueprint> {
  const apiKey = process.env['OPENAI_API_KEY'];
  if (!apiKey) throw new Error('OPENAI_API_KEY is not configured in the backend .env file.');
...
Return this exact JSON schema:
{
  ...
  return JSON.parse(content) as AIFillBlueprint;
}

function buildFallbackFillBlueprint(
  payload: ParsedExistingDungonPayload,
  params: GenerateDungonParams,
  monsters: DBMonster[],
  candidates: PlacementCandidate[],
  ...
  const monsterPlacements: AIFillMonsterPlacement[] = [];
...
function validateFillBlueprint(
  blueprint: AIFillBlueprint,
  monsterCandidates: PlacementCandidate[],
  monsters: DBMonster[],
): AIFillBlueprint {
  ...
  const monsterPlacements = Array.isArray(blueprint.monsterPlacements)
    ? blueprint.monsterPlacements.filter((placement) => (
      validMonsterSquares.has(placement.square)
      && validMonsterIds.has(placement.monsterDbId)
      && !usedSquares.has(placement.square)
      && usedSquares.add(placement.square)
    )).slice(0, 12)
    : [];
...
function buildFilledDungonPayload(
  payload: ParsedExistingDungonPayload,
  params: GenerateDungonParams,
  blueprint: AIFillBlueprint,
  monsters: DBMonster[],
...
interface ManualMonsterPlacement {
  square: string;
  monsterDbId: number;
}

interface ManualItemPlacement {
  square: string;
  itemDbId: number;
}

interface ManualFillBlueprint {
  startDescription: string;
  startPlayerSees: string;
  monsterPlacements: ManualMonsterPlacement[];
  itemPlacements: ManualItemPlacement[];
}

function buildManualFillBlueprint(
  payload: ParsedExistingDungonPayload,
  params: GenerateDungonParams,
  monsterCandidates: PlacementCandidate[],
  itemCandidates: PlacementCandidate[],
  validMonsterIds: Set<number>,
  ...
  const monsterPlacements: ManualMonsterPlacement[] = [];
  const itemPlacements: ManualItemPlacement[] = [];
...
function buildManualFilledDungonPayload(
  payload: ParsedExistingDungonPayload,
  params: GenerateDungonParams,
  blueprint: ManualFillBlueprint,
  monsters: DBMonster[],
  items: DBItem[],
): Record<string, unknown> {
  const output = deepClone(payload.basePayload);
  ...
  const monsterList: unknown[] = [...existingMonsterList];
  const monsterPlacements: unknown[] = [...existingMonsterPlacements];
  const itemPlacements: unknown[] = [...existingItemPlacements];
  ...
  const placedItemIds = new Set<number>(itemPlacements.map((placement) => {
    const record = placement as { itemId: number };
...
// ── Main Export ────────────────────────────────────────────────────────────────
export async function generateDungon(params: GenerateDungonParams, rawDungonJson: unknown): Promise<Record<string, unknown>> {
  const existingPayload = parseExistingDungonPayload(rawDungonJson);
  const anchorRow = Number.isInteger(params.anchorRow) ? (params.anchorRow as number) : existingPayload.startpoint.row;
  const anchorColumn = Number.isInteger(params.anchorColumn) ? (params.anchorColumn as number) : existingPayload.startpoint.col;
  ...
  const blueprint = buildManualFillBlueprint(
    existingPayload,
    params,
    monsterCandidates,
    itemCandidates,
