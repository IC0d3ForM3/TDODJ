...
const target = process.argv.includes('--target=prod') || process.argv.includes('prod') ? 'prod' : 'dev';
...
async function main() {
  const { rows } = await pool.query(
    `SELECT DISTINCT assettype, count(id) as cnt FROM images GROUP BY assettype ORDER BY cnt DESC`
  );
  console.table(rows);

  const { rows: itemRows } = await pool.query(
    `SELECT id, name, assettype FROM images WHERE name LIKE 'item-%' LIMIT 10`
  );
  console.log('\nSample item images:');
