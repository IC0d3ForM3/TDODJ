...
export class Potions implements OnInit {
  ...
  readonly userPotionForm = new FormGroup({
    ...
    effectAmountMin: new FormControl<number>(0, { nonNullable: true }),
    effectAmountDiceCount: new FormControl<number>(0, { nonNullable: true }),
    effectAmountDiceSides: new FormControl<number>(0, { nonNullable: true }),
    effectAmount2Min: new FormControl<number>(0, { nonNullable: true }),
  ...
  editPotion(item: UserPotionListItem): void {
    ...
    this.userPotionForm.reset({
      ...
      effectAmountMin: this.normalizeNumber(item.effectAmountMin ?? 0, 0),
      effectAmountDiceCount: Math.max(0, this.normalizeNumber(item.effectAmountDiceCount ?? (item.effectAmount > 0 ? 1 : 0), 0)),
      effectAmountDiceSides: Math.max(0, this.normalizeNumber(item.effectAmountDiceSides ?? item.effectAmount, 0)),
      effectAmount2Min: this.normalizeNumber(item.effectAmount2Min ?? 0, 0),
      effectAmount2DiceCount: Math.max(0, this.normalizeNumber(item.effectAmount2DiceCount ?? (item.effectAmount2 > 0 ? 1 : 0), 0)),
      effectAmount2DiceSides: Math.max(0, this.normalizeNumber(item.effectAmount2DiceSides ?? item.effectAmount2, 0)),
  ...
  private buildPayload(): UserPotionWritePayload {
    ...
    return {
      ...
      effectAmount: Math.max(0, this.normalizeNumber(c.effectAmountDiceSides.value, 0)),
      effectAmount2: Math.max(0, this.normalizeNumber(c.effectAmount2DiceSides.value, 0)),
      effectAmountMin: this.normalizeNumber(c.effectAmountMin.value, 0),
      effectAmountDiceCount: Math.max(0, this.normalizeNumber(c.effectAmountDiceCount.value, 0)),
      effectAmountDiceSides: Math.max(0, this.normalizeNumber(c.effectAmountDiceSides.value, 0)),
      effectAmount2Min: this.normalizeNumber(c.effectAmount2Min.value, 0),
      effectAmount2DiceCount: Math.max(0, this.normalizeNumber(c.effectAmount2DiceCount.value, 0)),
      effectAmount2DiceSides: Math.max(0, this.normalizeNumber(c.effectAmount2DiceSides.value, 0)),
