...
export class Items implements OnInit {
  ...
  readonly userItemForm = new FormGroup({
    ...
    minMindToRead: new FormControl<number>(0, { nonNullable: true }),
  ...
  editItem(item: UserItemListItem): void {
    ...
    this.userItemForm.reset({
      ...
      note: item.note ?? '',
      minMindToRead: this.normalizeNumber(item.minMindToRead, 0),
  ...
  private buildPayload(): UserItemWritePayload {
    ...
    return {
      ...
      minMindToRead: Math.max(0, this.normalizeNumber(c.minMindToRead.value, 0)),
